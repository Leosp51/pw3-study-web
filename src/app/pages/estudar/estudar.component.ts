...
export class EstudarComponent {
  constructor(private disciplinaService:DisciplinaService){
    disciplinaService.buscarDisciplinas().subscribe(resp=>{
      this.disciplinas = resp.map(i=>{ //map => conversão de um objeto em outro
        return{
          value: i.id,
          label: i.nome
        }
 ...
 //localhost:4020 front-end
 //localhost:8080 back-end
 //existe um mecanismo de proteção => não permite que ele faça requisições de uma origem (porta neste caso) para outra origem
 //(politica CORS causa esse bloqueio)
}
